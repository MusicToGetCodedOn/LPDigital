...
function ProjectDetail() {
    ...
    fetch(`${API_BASE_URL}/api/projects/${id}`)
      ...
      .then((data) => {
        ...
        const initialSituation =
          ...
          data.abstract?.initialSituation ||
          data.problem;
        const implementation =
          data.implementation || data.abstract?.implementation || data.solution;
        const results = data.results || data.abstract?.results || data.result;
        const learnings = data.learnings || data.abstract?.learnings;
        const technologies =
          data.technologies ||
          (Array.isArray(data.abstract?.technologies)
            ? data.abstract.technologies.join(", ")
            : data.abstract?.technologies);
  ...
  if (loading) {
    return (
      <div className="detail-loading">Projekt-Abstract wird geladen...</div>
    );
  }
  ...
  const handleDownload = async (e, url, filename) => {
    ...
    try {
      ...
      link.href = blobUrl;
      // Dateiname aus URL extrahieren oder Fallback nutzen
      link.download = filename || url.split("/").pop() || "Abstract.pdf";
      ...
      // Fallback: Öffnen im neuen Tab, falls Fetch blockiert wird
          ...
          <div className="paper-column">
            ...
            <section className="paper-section">
              ...
              <p className="section-subtext">
                Links zu relevanten Dokumentationen, Repositories oder
                Live-Demos für eine tiefere Analyse des Projekts.
              </p>
                ...
                {project.documentUrl && (
                  <button
                    ...
                    onClick={(e) =>
                      handleDownload(
                        e,
                        `${API_BASE_URL}${project.documentUrl}`,
                        project.documentUrl.split("/").pop(),
                      )
                    }
